...
// *  BUT PAGE   : -------------------------------------------------------*
// *                                                                      *
// *  Gestion des produits pour les points jaunes                         *
// *----------------------------------------------------------------------*
// *  MODIFICATIONS                                                       *
// *                                                                      *
// *                                                                      *
...
if(!empty($_POST["ajout"])){
    ...
    $tableau = array('codeabap' => $_POST["codeabap"],
                    'nomproduit' => $_POST["nomproduit"],                
                    'valpointsjaune' => $_POST["valpointsjaune"],                
                    'col' => $_POST["col"],                
                    'actif' => $az                
    );
    // Insertion des dites données
    insertSql("produits",$tableau);
    // Vide la variable POST
    unset($_POST["ajout"]);
    ?>
            <script>
            form = document.createElement("form");
            form.action = "/index.php?content=ajoutProduit";
            form.method = "post";
...
// si le champ "choix_1" de la variable post contient quelque chose
    ...
    foreach ($_POST as $champ => $value) {
        ...
        if ($value == "oui") 
        {
            ...
            $sql = "UPDATE produits SET actif = '1' WHERE id = ".$explosion[1].";";
        }
        // Si la valeur est égale à non
        if ($value == "non") 
        {
            // Met à jour l'article en le rendant inactif
            $sql = "UPDATE produits SET actif = '0' WHERE id = ".$explosion[1].";";
        }
...
<!-- Affichage de texte-->
    ...
    <div class="row text-center  h3">
        <b style="margin-top:5vh;" class ="text-warning">
        Vous êtes sur la page de gestion des produits jaune
        </b>
    ...
    <div class="row">
        <div class="col-3 "></div>
        <div class="col-6">
            <form action="/index.php?content=ajoutProduit" method="post" id="formChoix">
                <br>
                <div>
...
<?php
    ...
    foreach ($table as $ligne) {
        ...
        $a = array(
            ...
            "Valeurs en points" => $ligne["valpointsjaune"],
    ...
    ?>
        ...
        <div class="row">
            <div>
                <form action="/index.php?content=ajoutProduit" method="post" id="majActif">
...
?>
    ...
    <div class="row">
        <div class="col-3 "></div>
        <div class="col-6">
            <form action="/index.php?content=ajoutProduit" method="post" id="formAjoutProduit">
                ...
                <br>
                <div>
                    <label for="valpointsjaune">Valeur en point jaune</label>
                    <input type="number" class="form-control" placeholder="0" name="valpointsjaune" required>
                </div>
                <br>
                <div>
            ...
            <div class="text-center">
                <button class="btn btn-secondary btn-lg" type="submit" form="formAjoutProduit" value="Submit">Valider</button>
                <br><br>
            </div>
        </div>
        <div class="col-3"></div>
    ...
    <div class="row">
        <div class="col d-flex justify-content-center">
            <div class="card" style="width: 25rem;">
                <div class="card-body">
                    <h5 class="card-title">Veuillez sélectionner le fichier que vous souhaitez uploader :</h5>
                    <h6 class="card-subtitle mb-2 text-muted">Merci d'utilisez uniquement des fichiers CSV</h6>
                    <p class="card-text">
                        <div class="row text-center">
                            <div>
                                <form action="index.php?content=ajoutProduit" method="post" enctype="multipart/form-data">
            ...
            foreach ($nouveauxProduits as $ligne) {
                ...
                if($a === 0){
                    $affichage = array(
                        "Code SAP" => $ligne["codeabap"],
                        "Nom du produit" => $ligne["nomproduit"],
                        "Point Jaune" => $ligne["valpointsjaune"],
                        "Nombre de col par carton" => $ligne["col"],
                    );
                    array_push($joliTableau, $affichage);
                    insertSql("produits",$ligne);        
                }
            }
            //echo "<pre>";
